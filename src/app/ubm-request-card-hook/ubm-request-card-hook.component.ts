  ...
  @Input() set hostComponent(value: any) {
    if (value) {
      ...
      if (Array.isArray(relations)) {
        ...
        console.log("--- Requests : Valeurs de Target Relation ---", targetRelation);

        if (targetRelation) {
          // 1. Regex pour capturer tout ce qui est après $$V jusqu'au prochain $$
          const match = targetRelation.match(/\$\$V([^$]+)/);
          console.log("--- Requests : Valeurs du Match ---", match);
          ...
          if (match && match[1]) {
            ...
            console.log("--- Requests : Valeurs du Match ---", parts);

            const titreRevue = parts[0] || rawData;
            let refsHtml = '';

            if (parts.length >= 3) {
              refsHtml = `
                <span class="custom-revue-separator">. Voici les références du fascicule :</span>
                <span class="custom-revue-refs">${parts[1]} (${parts[2]})</span>
              `;
            } else if (parts.length === 2) {
              refsHtml = `
                <span class="custom-revue-separator">. Voici les références du fascicule :</span>
                <span class="custom-revue-refs">N° ${parts[1]}</span>
              `;
            }
